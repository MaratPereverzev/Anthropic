...
4. **Scan the `.claude/` directory** (run `find .claude -type f | sort`):
   - `skills/*/SKILL.md` — each is an invocable slash command; read the `name:` and `description:` frontmatter to write a one-line summary
   - `docs/**/*.md` — Claude-optimized reference docs; read the title line (`# ...`) and first blockquote (`> ...`) to write a one-line summary
   - `settings.json` / `settings.local.json` — project-level Claude configuration
   - Any other files found (commands, hooks, prompts, etc.)

   Collect every file path and its one-line description. These become the **Claude Resources** section in CLAUDE.md (see template below).
...
### Skills
@.claude/skills/<skill-name>/SKILL.md — <one-line description of what the skill does and when to invoke it>

### Documentation
@.claude/docs/<path>.md — <one-line description of what the doc covers>

### Settings
@.claude/settings.json — project-level Claude Code configuration (permissions, hooks, env)
```

**Writing descriptions for Claude Resources:**
- **Skills**: describe what the skill does and the trigger phrase. Example: `converts any file to a Claude-optimized .md doc — use with /convert-to-md <file>`
- **Docs**: describe the content and when a future Claude should load it. Example: `complete PostgreSQL column catalog — load when writing SQL or answering schema questions`
- **Settings**: one line is enough — just note what it configures
- Omit files that have no value to a future Claude (e.g., lock files, auto-generated artifacts)
...
**DO NOT omit:**
- The **Claude Resources** section — every `.claude/` file that exists must be listed with an `@<path>` reference and a one-line description. A future Claude that doesn't know these files exist cannot use them.
...
- [ ] Claude Resources section present with `@<path>` references to every `.claude/` file that a future Claude should know about
- [ ] Every `@<path>` reference has a one-line description on the same line (after ` — `)
- [ ] Every referenced path actually exists (verify with `Read` or `ls`)
